...
import com.i2asolutions.athelo.presentation.ui.base.chart.BarChart
...
fun HeartRateScreen(viewModel: HeartRateViewModel) {
    ...
    BoxScreen(
        ...
        SleepDetailsScreen(
            ...
            monthlyInformation = state.value.monthlyInformation,
...
fun SleepDetailsScreen(
    handleEvent: (HeartRateEvent) -> Unit,
    ...
    monthlyInformation: ActivityGraphScreen.ActivityLineInformation?,
    ...
    Column(
        ...
        ToolbarWithNameBack(
            backClick = { handleEvent(HeartRateEvent.BackClick) },
            screenName = stringResource(id = R.string.Heart_Rate)
        )
        RadioButtonGroup(
            ...
            buttons = arrayOf(
                RadioButton(
                    ...
                    onClick = {
                        handleEvent(HeartRateEvent.RangeChanged(HistoryRange.Day))
                    }
                ),
                RadioButton(
                    includePadding = false,
                    ...
                    onClick = {
                        handleEvent(HeartRateEvent.RangeChanged(HistoryRange.Week))
                    }),
                RadioButton(
                    includePadding = false,
                    text = stringResource(id = R.string.Month),
                    onClick = {
                        handleEvent(HeartRateEvent.RangeChanged(HistoryRange.Month))
                    })
            )
        )
        SwipeRefresh(
            state = rememberSwipeRefreshState(isRefreshing = false),
            onRefresh = { handleEvent(HeartRateEvent.RefreshData) }) {
            LazyColumn(
                modifier = Modifier
                    .fillMaxSize()
                    .padding(top = 24.dp),
                ...
                if (periodInfo != null) item {
                    PeriodInfoCell(
                        ...
                        prevClick = { handleEvent(HeartRateEvent.PrevClicked) },
                        nextClick = { handleEvent(HeartRateEvent.NextClicked) }
                    )
                }
                if (selectedRange == HistoryRange.Month && monthlyInformation != null) item {
                    LineGraphCell(monthlyInformation, selectedRange)
...
@Composable
fun LineGraphCell(info: ActivityGraphScreen.ActivityLineInformation, range: HistoryRange) {
    Column(
        modifier = Modifier
            .fillMaxWidth()
            .padding(horizontal = 16.dp)
        ...
        Text(
            ...
            text = stringResource(id = R.string.bps),
...
@Composable
fun BarGraphCell(info: ActivityGraphScreen.ActivityInformation, range: HistoryRange) {
    Column(
        modifier = Modifier
            .fillMaxWidth()
            .padding(horizontal = 16.dp)
