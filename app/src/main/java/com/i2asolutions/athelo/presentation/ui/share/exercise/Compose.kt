...
import com.i2asolutions.athelo.presentation.ui.share.activity.ActivityEvent
...
fun SleepDetailsScreen(
    handleEvent: (ExerciseEvent) -> Unit,
    ...
    Column(
        ...
        ToolbarWithNameBack(
            backClick = { handleEvent(ExerciseEvent.BackClick) },
            screenName = stringResource(id = R.string.Exercises)
        )
        ...
        RadioButtonGroup(
            ...
            buttons = arrayOf(
                RadioButton(
                    ...
                    onClick = {
                        handleEvent(ExerciseEvent.RangeChanged(HistoryRange.Day))
                    }
                ),
                RadioButton(
                    includePadding = false,
                    ...
                    onClick = {
                        handleEvent(ExerciseEvent.RangeChanged(HistoryRange.Week))
                    }),
                RadioButton(
                    includePadding = false,
                    text = stringResource(id = R.string.Month),
                    onClick = {
                        handleEvent(ExerciseEvent.RangeChanged(HistoryRange.Month))
                    })
            )
        )
        SwipeRefresh(
            state = rememberSwipeRefreshState(isRefreshing = false),
            onRefresh = { handleEvent(ExerciseEvent.RefreshData) }) {
            LazyColumn(
                modifier = Modifier
                    .fillMaxSize()
                    .padding(top = 24.dp),
                ...
                if (periodInfo != null) item {
                    PeriodInfoCell(
                        ...
                        prevClick = { handleEvent(ExerciseEvent.PrevClicked) },
                        nextClick = { handleEvent(ExerciseEvent.NextClicked) }
                    )
                }
    ...
    if (showPatientChooser && patients.isNotEmpty()) {
        ChoosePatientPopup(
            ...
            onSwitchClick = { patient ->
                handleEvent(ExerciseEvent.ChangePatient(patient))
                showPatientChooser = false
            })
    }
}
