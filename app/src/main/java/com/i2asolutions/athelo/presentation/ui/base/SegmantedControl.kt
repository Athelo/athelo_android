...
private val TRACK_COLOR = lightGray.copy(alpha = 0.3f)
private val THUMB_COLOR = background
 ...
 */
@Composable
fun SegmentText(text: String, textStyle: TextStyle = TextStyle()) {
    Text(text, maxLines = 1, overflow = Ellipsis, style = textStyle)
}
...
private fun Thumb(state: SegmentedControlState) {
    Box(
        Modifier
            ...
            .background(THUMB_COLOR, BACKGROUND_SHAPE)
    )
}
...
private fun <T> Segments(
    ...
    CompositionLocalProvider(
        LocalTextStyle provides TextStyle(fontWeight = FontWeight.Normal)
    ) {
        Row(
